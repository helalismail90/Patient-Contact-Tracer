class Api::Routes::RoutesController < Api::AbstractApiController
  ...
  def get_route(pointA, pointB)
    requestURL = "http://router.project-osrm.org/route/v1/driving/#{pointA[:latitude]},#{pointA[:longitude]};#{pointB[:latitude]},#{pointB[:longitude]}?overview=full&geometries=geojson"
